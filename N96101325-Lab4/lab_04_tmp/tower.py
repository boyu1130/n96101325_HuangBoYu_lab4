...
class Circle:
    ...
    def collide(self, enemy):
        ...
        ...
        """
        x1, y1 = enemy.get_pos()  #判斷是否進入塔的範圍
        (x, y) = self.center
        
        if math.sqrt((x1-x)**2+(y1-y)**2) <= self.radius: #使用math.sqrt算出小兵與塔心的距離
            return True
    ...
    def draw_transparent(self, win):
        ...
        :return: None
        """
        transparent_surface = pygame.Surface((500, 500), pygame.SRCALPHA)   #設定矩形大小
        transparency = 50   #透明度
        
        pygame.draw.circle(transparent_surface, (255 , 255 , 255, transparency), (self.radius,self.radius),self.radius)   #在矩形上畫圓
        
        x,y = self.center        #定義 X, Y
        win.blit(transparent_surface,(x-self.radius,y-self.radius))    #畫!! 
...
class Tower:
    ...
    def is_cool_down(self):
        ...
        :return: Bool
        """
        if self.cd_count >= self.cd_max_count:        #用判斷式設定冷卻時間
            self.cd_count = 0
            return True
        else:
            self.cd_count += 1
    ...
    def is_clicked(self, x, y):
        ...
        :return: Bool
        """
        #判斷是否點在塔的範圍內,這裡設定為攻擊範圍的1/3
        (x1,y1)=self.rect.center
        if math.sqrt((x1-x)**2+(y1-y)**2) <= self.range/3 :
            return True
        else:
            return False
